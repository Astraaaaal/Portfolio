...
const decorationMap = {
  neo_blocks: NeoBlocks,
  synthwave_grid: SynthwaveGrid,
  constructivist_stripe: ConstructivistStripe,
  doodles: DoodleElements,
...
export default function Hero() {
  ...
  const Decoration = cfg.heroDecoration ? decorationMap[cfg.heroDecoration] : null;

  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      {/* Gradient blobs — only for themes that use them */}
      {cfg.showBlobs && (
      ...
      {/* Grid pattern — only for flat design */}
      {cfg.showGridPattern && (
      ...
      {/* Floating shapes — only for flat design */}
      {cfg.showFloatingShapes && (
      ...
      {/* Content — optionally wrapped */}
      {cfg.heroWrapper === 'win95' ? (
        <Win95Window title="Portfolio.exe">
          <HeroContent t={t} />
        </Win95Window>
      ) : (
        <HeroContent t={t} />
